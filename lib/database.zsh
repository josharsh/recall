#!/usr/bin/env zsh
# database.zsh - SQLite database operations for project-memory

# Initialize database schema
_projmem_db_init() {
  # Check if sqlite3 is available
  if ! command -v sqlite3 &> /dev/null; then
    echo "Warning: sqlite3 not found. Project Memory requires sqlite3."
    return 1
  fi

  # Create schema if not exists
  sqlite3 "$PROJMEM_DB" <<EOF
...
_projmem_db_get_top_commands() {
  local project_path="$1"
  local limit="${2:-10}"
  local days="${3:-30}"
  local cutoff=$(( $(date +%s) - (days * 86400) ))

  sqlite3 -separator $'\t' "$PROJMEM_DB" <<EOF
SELECT
  c.command,
  COUNT(*) as count,
